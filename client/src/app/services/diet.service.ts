...
//   addDiet(newDiet: Diet): Observable<Diet> {
//     return this.http.post<Diet>(this.apiUrl, newDiet);
//   }

//   updateDiet(updatedDiet: Diet): Observable<Diet> {
//     return this.http.put<Diet>(`${this.apiUrl}/${updatedDiet.id}`, updatedDiet);
//   }

//   deleteDiet(dietId: string): Observable<any> {
//     return this.http.delete(`${this.apiUrl}/${dietId}`);
//   }
// }
...
import { Diet } from '../models/diet';
import { environment } from '../environments/environment';
import { DietToAdd } from '../models/diet-to-add';
import { DietToEdit } from '../models/diet-to-edit';
...
export class DietService {
  ...
  // Fetch diets for a specific user by searching with userId
  fetchDietsForUser(userId: string): Observable<Diet[]> {
    const url = this.baseUrl + 'diets/search';
    let params = new HttpParams().set('userId', cliendId);
